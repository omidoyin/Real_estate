...
export default function PaymentPlan() {
  ...
  const handleMakePayment = (planId, installmentId) => {
    ...
    alert(`Processing payment for plan ${planId}, installment ${installmentId}`);
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      {paymentPlans.length === 0 ? (
        <div className="bg-light-blue p-8 rounded-lg text-center">
          <h2 className="text-xl font-semibold mb-4">You don't have any payment plans yet.</h2>
          <p className="mb-6">Browse our available properties and make a purchase to start a payment plan.</p>
          <Link 
              ...
              <div className="p-6">
                <div className="flex flex-col md:flex-row md:items-center mb-6">
                  ...
                  <div className="md:w-3/4 md:pl-6">
                    <h2 className="text-xl font-semibold mb-2">
                      <Link href={`/lands/details/${plan.propertyId}`} className="hover:text-primary-blue">
                        {plan.propertyTitle}
                      </Link>
                    </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">${plan.paidAmount.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-gray-600 text-sm">Remaining Amount</p>
                        <p className="font-semibold">${plan.remainingAmount.toLocaleString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {plan.paymentStatus === 'In Progress' && (
                        <div className="text-right">
                          ...
                          <p className="font-semibold">${plan.nextPaymentAmount.toLocaleString()} on {new Date(plan.nextPaymentDate).toLocaleDateString()}</p>
                        </div>
                      )}
                    </div>
                  </div>
                        ...
                        {plan.installments.map((installment, index) => (
                          <tr key={installment.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                              {installment.status === 'Due' ? (
                                <button
                                  onClick={() => handleMakePayment(plan.id, installment.id)}
                ...
                {plan.paymentStatus === 'In Progress' && (
                  <div className="mt-6 flex justify-end">
                    <button
                      onClick={() => handleMakePayment(plan.id, plan.installments.find(i => i.status === 'Due')?.id)}
      ...
      <div className="mt-8 bg-light-blue p-6 rounded-lg">
        <h2 className="text-xl font-semibold mb-4">Payment Information</h2>
        <p className="mb-4">For any questions regarding your payment plan or to discuss alternative payment options, please contact our finance team:</p>
